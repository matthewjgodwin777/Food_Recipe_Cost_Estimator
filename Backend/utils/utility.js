function getActualQuantity(inputString) {
...
function extractListsFromAIResponse(aiResponse) {
    // Regular expression to match lists in the response
    const listRegex = /\[([^\]]+)\]/g;

    // Array to store the extracted lists
    ...
    while ((match = listRegex.exec(aiResponse)) !== null) {
        ...
        tuples.forEach(tuple => {
            ...
            names.push(parts[0]); // First value is the name
            quantities.push(parseFloat(parts[1])); // Second value is the quantity (convert to number)
            units.push(parts[2]); // Third value is the unit
        });

        extractedLists.push(names, quantities, units);
    }

    // Ensure we have exactly 3 lists
    if (extractedLists.length !== 3) {
        throw new Error("AI response does not contain exactly 3 lists.");
    }

    console.log("Extracted lists from AI response:", extractedLists);

    return extractedLists;
}

function convertToRequiredUnitOfQty(actualUnit, quantity, quantityUnit) {
...
function calculateCostOfIngredients(item) {
    console.log("-----------------------------------------------------------------------------------------------------------");
...
function convertToApproximateUnitOfQty(actualUnit, quantity, quantityUnit) {
